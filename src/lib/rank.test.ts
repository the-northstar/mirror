...
describe('what counts as recommended, aisle by aisle', () => {
  // The shelf is ordered, not filtered, so "500 ranked" counted a foundation
  // 22 ΔE off her face. That number described the catalogue, not her.
  test('foundation: a match in most light, not merely the nearest', () => {
    const shelf: Product[] = [
      { id: 'match', aisle: 'foundation', brand: 'A', name: 'Match', hex: '#3a2a20', colorName: 'brown' },
      { id: 'miles-off', aisle: 'foundation', brand: 'B', name: 'Off', hex: '#f0e8e0', colorName: 'pale' },
    ]
    const out = rankFoundation(shelf, '#3a2a20')
    expect(out.find((p) => p.id === 'match')!.recommended).toBe(true)
    expect(out.find((p) => p.id === 'miles-off')!.recommended).toBe(false)
  })

  test('lipstick: a wearable hue that actually moves her mouth', () => {
    const out = rankLipstick(
      swatchShelf('lipstick', {
        olive: '#6b6b3a', invisible: '#a9585a', barely: '#8c2f3f', good: '#6d1f33',
      }),
      shopperFor('#e0b68f', { lipHex: '#a9585a' }),
    )
    const by = (id: string) => out.find((p) => p.id === id)!
    expect(by('good').recommended).toBe(true)        // shifts her lips 23 ΔE
    expect(by('olive').recommended).toBe(false)      // not a lip hue at all
    expect(by('invisible').recommended).toBe(false)  // her own lip colour
    // Wearable, and still not a recommendation: 15 ΔE is a tint, not a change.
    // Clearing the gate is not the same as suiting her.
    expect(by('barely').recommended).toBe(false)
  })

  test('blush: visible without sitting on the face', () => {
    const out = rankBlush(
      swatchShelf('blush', { flush: '#c9697a', vanishes: '#bd987d', stripe: '#ff1744' }),
      shopperFor('#bd987d'),
    )
    const by = (id: string) => out.find((p) => p.id === id)!
    expect(by('flush').recommended).toBe(true)
    expect(by('vanishes').recommended).toBe(false)
    expect(by('stripe').recommended).toBe(false)
  })

  test('clothes: nothing the ranker would warn her about', () => {
    const flushed = shopperFor('#bd987d', {
      concerns: [{ type: 'redness', ui_score: 30, raw_score: 25 }],
    })
    const out = rankClothes(
      swatchShelf('clothes', { petrol: '#2f5d62', scarlet: '#c8322d', washedOut: '#bd987d' }),
      flushed,
    )
    const by = (id: string) => out.find((p) => p.id === id)!
    expect(by('petrol').recommended).toBe(true)
    expect(by('scarlet').recommended).toBe(false)    // amplifies measured redness
    expect(by('washedOut').recommended).toBe(false)  // her own lightness
  })

  test('skincare: it aims at something her scan actually flagged', () => {
    const out = rankSkincare(SNAPSHOT.skincare, [
      { type: 'oiliness', ui_score: 20, raw_score: 18 },
    ])
    expect(out[0].recommended).toBe(true)
    expect(out.at(-1)!.recommended).toBe(false)
    // With nothing measured, nothing can be recommended FOR her.
    for (const p of rankSkincare(SNAPSHOT.skincare, [])) expect(p.recommended).toBe(false)
  })

  test('hair: an oval face carries everything, an unread face carries nothing', () => {
    const styles = ['Blunt Bob', 'Long Layers'].map((name) => ({
      id: name, aisle: 'hair' as Aisle, brand: 'Female', name,
      hex: '#e8e4dd', colorName: 'neutral', score: 0, reason: '', recommended: false,
    }))
    for (const s of rankHair(styles, 'oval')) expect(s.recommended).toBe(true)
    // No face shape means no claim either way, so none are counted as hers.
    for (const s of rankHair(styles, undefined)) expect(s.recommended).toBe(false)
    // And a shape with a real preference splits them.
    const round = rankHair(styles, 'round')
    expect(round.find((s) => s.name === 'Long Layers')!.recommended).toBe(true)
    expect(round.find((s) => s.name === 'Blunt Bob')!.recommended).toBe(false)
  })
})
